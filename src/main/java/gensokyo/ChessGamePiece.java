...
public abstract class ChessGamePiece implements Serializable{
   ...
   protected ArrayList<String> calculateDiagonalMoves(ChessGameBoard board, int numMoves, DiagonalStates direction) {
       ...
       if ( isPieceOnScreen() ){
           for ( int i = 1; i < 8 && count < numMoves; i++ ){
            ...
            if ( isAddMove(targetRow,targetCol, board) ) {
           		moves.add( ( targetRow ) + "," + ( targetCol ) );
                count++;
           	}  
            else 
            {
            	if(isEnemy( board, targetRow, targetCol ) ) {
            		moves.add( ( targetRow ) + "," + ( targetCol ) );
            	}
                break;
           	}
   ...
   protected boolean isAddMove(int targetRow, int targetCol, ChessGameBoard board) {
	   return isOnScreen( targetRow, targetCol ) && board.getCell( targetRow, targetCol ).getPieceOnSquare() == null;
   }
